...
Usage:
  python3 -m scripts.run_crawler --source baodauthau --timeframe 1w
  python3 -m scripts.run_crawler --all --timeframe 1m
"""
...
async def main():
    ...
    if args.all or not args.source:
        print("[*] Crawling ALL active sources within the selected timeframe...")
        runs = await crawler_service.run_all_sources(force_recrawl=args.force, timeframe=args.timeframe)
        for r in runs:
        ...
        print(f"[*] Crawling source '{args.source}' within the selected timeframe...")
        run = await crawler_service.run_crawler_for_source(args.source, force_recrawl=args.force, timeframe=args.timeframe)
